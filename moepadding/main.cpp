...
void singleQuoteContext(SuperString& from, std::string& to)
{
    ...
    while (!from.is_exhausted())
    {
        ...
        {
            from.append_strike(to, 1); // Consume the escape sequence character
            from.append_strike(to, 1); // Can only be one character here
        }
        else
        {
            from.append_strike(to, 1);
...
std::string siftOuterScopes(std::string const& s)
{
    ...
    for (char const& c : s)
    {
        assert(isWhitespace(c) || c == '{' || c == '}' || c == ';' || c == 'x');
                      ...
                      int wantPadding)
{
    while (!stringMarkers.empty())
    {
        ...
        std::size_t const newlines = occurrences(initial_window, "\r\n");
        assert(newlines > 0);
